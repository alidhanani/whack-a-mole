...
import { Board, BoardBackground, Prompt, Screenlabel } from 'src/components';

import { HOLE, MOLE } from '../constant/Constant';
...
function Main() {
  ...
  useEffect(() => {
    ...
    function updateValue() {
      const guessNumber = randomNumberInRange(0, 11);
  ...
  const onArrayClick = (event: any) => {
    if (event.target.src === '' + MOLE) {
      setScore((oldvalue) => oldvalue + 1);
    }
  };

  const rowElements1 = rowElem1.map((element) => (
    <td
      style={{
    ...
    >
      <Image
        radius='xs'
        width={200}
        height={pic[element] === MOLE ? 150 : 50}
        src={'' + pic[element]}
      />
    </td>
  ));

  const rowElements2 = rowElem2.map((element) => (
    <td
      style={{
        height: window.innerHeight / 5,
        width: window.innerWidth / 6,
  ...
  const rowElements3 = rowElem3.map((element) => (
    <td
      style={{
        height: window.innerHeight / 5,
        width: window.innerWidth / 6,
  ...
  return (
    <BoardBackground
      Data={
        <>
          <Prompt
            FinalModalOpen={opened}
            FinalOnClose={() => setOpened(false)}
            FinalModalOnPress={() => {
              setOpened(false);
              setOpenedInit(true);
            }}
            Data={scoreArray
              ...
              .reverse()
              .slice(0, 9)}
            InitModal={openedInit}
            InitClose={() => setOpenedInit(false)}
            InitButtonPress={() => {
              setScore(0);
              restart(time);
              setOpenedInit(false);
            ...
            ButtonDisable={name === '' ? true : false}
          />
          <Screenlabel Score={score} Time={[minutes, seconds]} />
          <Board
            Cursor={WAM_Hammer}
            Data={[rowElements1, rowElements2, rowElements3]}
          />
        </>
      }
    />
